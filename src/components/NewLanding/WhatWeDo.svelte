<script lang="ts">
  ...
  // Массив карточек, где вместо статических строк указываем ключи для перевода
  const cards = [
    {
      titleKey: 'whatWeDo.cards.modernization.title',
      descriptionKey: 'whatWeDo.cards.modernization.description'
    },
    {
      titleKey: 'whatWeDo.cards.creation.title',
      descriptionKey: 'whatWeDo.cards.creation.description'
    },
    {
      titleKey: 'whatWeDo.cards.digitalization.title',
      descriptionKey: 'whatWeDo.cards.digitalization.description'
    }
  ];
</script>
...
<div class="container" id="what-we-do-section">
  ...
  <h1 class="title">{$t('whatWeDo.title')}</h1>
  <p class="description">{$t('whatWeDo.description')}</p>
  ...
  <div class="cards-container">
    {#each cards as card, index}
      ...
      >
        ...
        <p class="card-description">{$t(card.descriptionKey)}</p>
      </div>
    {/each}
  </div>
</div>
