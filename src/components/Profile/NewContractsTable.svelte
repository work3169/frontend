<script lang="ts">
  ...
  /* --- i18n --- */
  import i18next from "../../lib/i18n";            /* подкорректируйте путь при необходимости */
    ...
    <table class="table table-compact w-full">
      <thead class="text-neutral-content">
        <tr>
          ...
          <th class="bg-neutral text-xs md:text-md">{$t('contractTable.name')}</th>
          <th class="bg-neutral text-xs md:text-md text-right">{$t('contractTable.min')}</th>
          <th class="bg-neutral text-xs md:text-md text-right">{$t('contractTable.max')}</th>
          <th class="bg-neutral text-xs md:text-md text-right">{$t('contractTable.percent')}</th>
          <th class="bg-neutral text-xs md:text-md text-right">{$t('contractTable.term')}</th>
        </tr>
      </thead>
      <tbody>
        {#each contracts as contract}
          <tr>
            ...
            <td class="text-right">{contract.term} d.</td>
          </tr>
        {/each}
      </tbody>
    </table>
