<script lang="ts">
  ...
  const inviteAfterStart = async (contract_id: string | number, username: string) => {
      try {
          ...
          successMsg = $t('commonContract.inviteSuccess');
      } catch (e) {
          if (axios.isAxiosError(e)) {
              ...
              if (errMsg) {
                  ...
                  console.log(e);
                  errorMsg = $t('commonContract.serverError');
              }
          } else {
              console.log(e);
              errorMsg = $t('commonContract.serverError');
  ...
  const acceptInvitation = async (invitation: CommonContractItem) => {
    try {
      ...
      errorMsg = "";
      if (!amount) {
          errorMsg = $t('commonContract.enterAmount');
          return;
      }
      const accessToken = await user.getAccessToken();
      const formData = new FormData();
...
<div class="bg-white p-4 rounded-lg shadow-md space-y-2 max-w-2xl">
  <div class="flex items-center justify-between gap-1">
    <h3 class="flex items-center text-lg font-medium">
      ...
      {#if contract.is_active}
        <div class="ml-1 badge badge-secondary">
          {$t('commonContract.active')}
        </div>
      {/if}
    </h3>
    <div class="text-xs text-gray-400 text-right">
      {$t('commonContract.created')}: 
      <span class="text-sm text-black">{formatDate(contract.created_at)}</span>
      - 
      {$t('commonContract.ends')}: 
      <span class="text-sm text-black">{formatDate(contract.end_date)}</span>
    </div>
  </div>
  <div class="text-gray-400 text-sm">
      {$t('commonContract.creator')}: 
      <span class="text-black text-md">{contract.contract_creator}</span>
  </div>
  {#if isInvitation}
    <div class="flex flex-col gap-2">
      {#if !isAccepting && !isDeclining}
        <div class="flex gap-2">
          <button class="btn btn-primary" on:click={() => (isAccepting = true)} disabled={isLoading}>
            {$t('commonContract.accept')}
          </button>
          <button class="btn btn-outline" on:click={() => (isDeclining = true)} disabled={isLoading}>
            {$t('commonContract.decline')}
          </button>
        </div>
      {/if}
      {#if isDeclining}
        <div class="flex flex-col gap-2">
          {$t('commonContract.confirmDecline')}
      ...
      {#if isAccepting}
        <div class="flex flex-col gap-2">
          {$t('commonContract.enterContribution')}
          <input
            bind:value={amount}
            class="input input-bordered w-full max-w-sm"
            placeholder={$t('commonContract.amountPlaceholder')}
          />
          <div class="flex gap-2">
              <button
  ...
  {:else}
    <div class="text-gray-400 text-sm">
      {$t('commonContract.yourContribution')}: 
      <span class="text-md text-black font-semibold">{contract.amount}</span>
    </div>
    <div class="text-gray-400 text-sm">
      {$t('commonContract.plannedIncome')}: 
      <span class="text-md text-black font-semibold">{contract.dividends}</span>
    </div>
  {/if}
  {#if errorMsg}
  ...
  {#if isCreated}
      {#if !isAdding}
          <button class="btn btn-outline btn-secondary" on:click={() => (isAdding = true)}>
            {$t('commonContract.addUser')}
          </button>
      {:else}
          <input class="input input-bordered" bind:value={username} placeholder={$t('commonContract.usernamePlaceholder')} />
          <button class="btn btn-outline btn-secondary" on:click={async () => await inviteAfterStart(contract.contract_id, username)}>
            {$t('commonContract.addUser')}
          </button>
          <button
              class="btn btn-md btn-outline"
